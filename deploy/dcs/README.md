...
The mission has two unarmed ground groups and a small Lua adapter. Its separate Saved Games hook is installed by `./scripts/dcs.sh bridge`; it does not modify DCS installation files. See the [JSON bridge guide](../../docs/BRIDGE.md) for the protocol, test client and current validation status.
