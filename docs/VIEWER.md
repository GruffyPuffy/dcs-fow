...
Status: viewer implemented; live map display awaits a mission restart with the updated `fow.miz`.

The viewer is a small, read-only Python HTTP server. It asks the FoW/DCS JSON bridge for a snapshot every 10 seconds and serves the latest result to a browser. It does not send DCS orders. Browser refreshes read a cache, so opening another tab does not increase DCS polling. It binds to `127.0.0.1:8765` by default; the bridge remains on `127.0.0.1:10309`.
...
Select/restart `fow.miz` in the DCS WebGUI and unpause it. A DCS process restart is unnecessary for this mission-only change. Then run:
...
The viewer displays the raw, omniscient bridge state. Keep it local while fog-of-war filtering and login are not implemented. The 10-second poll is a trial setting, not a planned final simulation cadence. It can be changed with `--interval`.
